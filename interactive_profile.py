...
def safe_print(*args, sep=' ', end='\n'):
    try:
        ...
        print(text.encode(encoding, errors='replace').decode(encoding), end=end)
...
def power_message(age):
    ...
    return 'Tumhari sachchi shakti tumhari buddhi aur anubhav mein basti hai. abhi bhi tum nayi choti chadh sakte ho.'
...
def ask_questions():
    print('=== Bhagavad Gita + Numerology Profile ===')
    print('Zyada sawaal puchh raha hoon taaki tumhara profile aur gehra ho sake.')
    print()
    full_name = input('Tumhara poora naam kya hai? ').strip() or 'Mitra Sharma'
    age_text = input('Tumhari umr kitni hai? ').strip()
    try:
        age = int(age_text)
    except ValueError:
        age = 20
    gender = input('Tum ladka ho ya ladki? ').strip() or 'ladka'
    city = input('Tum kis sheher se ho? ').strip() or 'apna sheher'
    ambition = input('Tumhara sapna ya ichha kya hai? ').strip() or 'safalta paana'
    favorite_color = input('Tumhara pasandida rang kaun sa hai? ').strip() or 'neela'
    challenge = input('Aaj tumhari sabse badi chunauti kya hai? ').strip() or 'naya mauka dhundhna'
    career_goal = input('Tum kis kshetra mein kaam karna chahte ho? ').strip() or 'apna vyavasaay'
    birthdate = input('Tumhari janam tithi kya hai? (DDMMYYYY ya blank) ').strip()
    return full_name, age, gender, city, ambition, favorite_color, challenge, career_goal, birthdate


def print_gita_data():
    safe_print('\n--- Bhagavad Gita ke pramukh vishay ---')
    for topic in GITA_TOPICS:
        safe_print('-', topic)
    safe_print('\n--- Bhagavad Gita chapter themes ---')
    for number, theme in CHAPTER_THEMES.items():
        safe_print(f'Chapter {number}: {theme}')


def print_key_teachings():
    safe_print('\n--- Bhagavad Gita ke key teachings ---')
    for chapter, teachings in GITA_KEY_TEACHINGS.items():
        safe_print(f'Chapter {chapter}:')
        for teaching in teachings:
            safe_print(f'  - {teaching}')


def main():
    full_name, age, gender, city, ambition, favorite_color, challenge, career_goal, birthdate = ask_questions()
    safe_print('\n--- Tumhara Vishleshan ---')
    safe_print(profile_summary(full_name, age, gender, city, ambition, favorite_color, challenge, career_goal))
    ...
    if numbers['expression'] is not None:
        safe_print(f"Expression/Destiny number: {numbers['expression']} - {numerology_description(numbers['expression'])}")
    if numbers['soul'] is not None:
        safe_print(f"Soul urge number: {numbers['soul']} - {numerology_description(numbers['soul'])}")
    if numbers['personality'] is not None:
        safe_print(f"Personality number: {numbers['personality']} - {numerology_description(numbers['personality'])}")
    if life_path is not None:
        safe_print(f"Life path number: {life_path} - {numerology_description(life_path)}")
    if maturity is not None:
        safe_print(f"Maturity number: {maturity} - {numerology_description(maturity)}")
    safe_print(f"Karmic lesson numbers missing in your name: {', '.join(missing) if missing else 'None - sabhi numbers majood hain.'}")

    safe_print('\n--- Bhagavad Gita Insights ---')
    safe_print(get_gita_advice(challenge, ambition))
    safe_print(get_topic_tagline(ambition, challenge))
    safe_print('Quote suggestion: ' + random.choice(GITA_QUOTE_PROMPTS))

    print_gita_data()
    print_chapter_previews()
    print_key_teachings()
